...
void displaySuggestions(const std::string& prefix, const PrefixSearcher& prefixSearcher) {
    // Note: getSuggestions expects a prefix without the '*'
    std::string clean_prefix = prefix.back() == '*' ? prefix.substr(0, prefix.size() - 1) : prefix;
...
int main() {
    ...
    // IMPORTANT: These files must exist in the same directory for indexing to work.
    ...
    while (true) {
        ...
        if (results.empty()) {
            ...
            for (size_t i = 0; i < results.size(); ++i) {
                ...
                int totalWords = indexer.getDocLen().count(doc) ? indexer.getDocLen().at(doc) : 0;
                ...
                while (iss >> word_token) {
                    if (word_token == "AND" || word_token == "OR" || word_token == "-") continue;
                    
                    std::string clean_word = word_token;
                    // Pre-clean for normalization logic
                    if (clean_word.front() == '"' && clean_word.back() == '"') clean_word = clean_word.substr(1, clean_word.size() - 2);
                    if (clean_word.back() == '*') clean_word.pop_back();

                    // Tokenize the (potentially cleaned) word/phrase to handle multi-word inputs
                    std::istringstream twiss(clean_word);
                    std::string token;
                    while (twiss >> token) {
                        std::string normalized_token = normalizeWord(token);
                        
                        if (!normalized_token.empty() && seen_terms.find(normalized_token) == seen_terms.end()) {
